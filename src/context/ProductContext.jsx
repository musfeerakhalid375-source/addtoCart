...
const addCart = (product)=>{
 setMassage(`${product.name} added to the cart`)
...
const removeCart = (product)=>{
 ...
 setMassage(`${product.name} remove from the cart`)
...
setInvoice(previous =>{
    ...
    useEffect(()=>{
        const timeout = setTimeout(()=> {setMassage('')},400)
        setInvoiceData();
    ...
    return (
        <ProductContext.Provider value={{ products , filterProducts , addCart , removeCart,  invoice ,cart }}>
           {message && <div className='fixed shadow-lg rounded right-0 top-20 bg-green-600 text-white min-w-[300px] p-2 text-center  '>{message}</div>}
